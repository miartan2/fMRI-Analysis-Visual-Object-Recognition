...
# This script:
# - Loads the Haxby dataset (Subject 1)
# - Applies brain masking, smoothing, high-pass filtering, and standardization
# - Builds a block-design events table from stimulus labels
# - Fits a first-level GLM with an SPM hemodynamic model
# - Computes the *face > house* contrast
# - Extracts regional time series using the AAL atlas
# - Computes a 116×116 functional connectivity matrix
# - Identifies the top hub regions in the brain
...
1. GLM Results (Face > House)
The face > house contrast produced significant activation (z > 3.0) in regions 
aligned with the known face-selective network:
- Bilateral fusiform gyrus (Fusiform Face Area)
- Inferior temporal cortex
- Occipital face-sensitive visual regions
These findings replicate classic results showing that the ventral visual stream 
specializes in face perception.

2. Design Matrix
The design matrix confirms a clean block design with alternating face and house 
conditions, enabling strong model estimation and clear category-selective 
responses.

3. Functional Connectivity
The most highly connected hubs included:
- Calcarine cortex (L/R)
- Lingual gyrus (L/R)
- Cerebellum Lobules 4–5 (L/R)
- Hippocampus (L/R)
- Medial superior frontal cortex
These hubs reflect coordinated responses to visual stimulation and involvement 
of memory and attentional networks.
